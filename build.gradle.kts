...
fun isDynamicVersion(version: String) =
  version.endsWith("+") || version.startsWith("latest.") || Regex("""[\[\](),]""").containsMatchIn(version)
    ...
    resolutionStrategy {
      eachDependency {
        if (requested.group != "org.pkl-lang" && isDynamicVersion(requested.version.toString())) {
          throw GradleException("Dynamic version '${requested.version}' is not allowed for ${requested.group}:${requested.name}")
        }
      }
    }
  }
